...
public class BlockMath1
{
	 ...
	 */
	public static void mipmapStuff(
			final Matrix4fc sourceToNDC,
			final int[] viewport,
			final int[] levelScaleFactors )
	{
		final MipmapSizes sizes = new MipmapSizes();
		sizes.init( sourceToNDC, viewport[ 3 ] );
		System.out.println( "sn = " + sizes.getSn() );
		System.out.println( "sf = " + sizes.getSf() );

		final float sl = sizes.sl( levelScaleFactors );
