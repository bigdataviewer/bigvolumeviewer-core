...
public class Example3 implements GLEventListener
{
	...
	public void display( final GLAutoDrawable drawable )
	{
		...
		if ( !freezeRequiredBlocks )
		{
			...
			final RequiredBlocks requiredBlocks = computeRequiredBlocks( pvm, 0 );
			levelStuff( raiLevels, pvm, requiredBlocks, offscreen.getWidth() );
...
//		screenPlane.draw( gl );
	...
	private void levelStuff( final List< RaiLevel > raiLevels, final Matrix4f pvm, final RequiredBlocks requiredBlocks, final int viewportWidth )
	{
		final MipmapSizes sizes = new MipmapSizes();
		sizes.init( pvm, viewportWidth );

		float[] sls = new float[ raiLevels.size() ];
		for ( int i = 0; i < raiLevels.size(); i++ )
		{
			RaiLevel raiLevel = raiLevels.get( i );
			sls[ i ] = sizes.sl( raiLevel.r );
		}

		final int[] r = raiLevels.get( 0 ).r;
		Vector3f x = new Vector3f();
		Vector3f temp = new Vector3f();
		HashSet< Integer > usedLevels = new HashSet<>();
		for ( int[] gridPos : requiredBlocks.getGridPositions() )
		{
			for ( int d = 0; d < 3; ++d )
				x.setComponent( d, ( gridPos[ d ] + 0.5f ) * blockSize[ d ] * r[ d ] );
			final int l = sizes.bestLevel( sls, x, temp );
			usedLevels.add( l );
		}

		System.out.println( "usedLevels = " + usedLevels );
		System.out.println();
	}
