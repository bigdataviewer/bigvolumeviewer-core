...
/**
 * Extreme wide-angle for oob and scale level visualization
 */
public class Example6 implements GLEventListener
{
	...
	public Example6( List< RaiLevel > raiLevels, final AffineTransform3D sourceTransform )
	{
	...
	private double dCam = 400;
	private double dClipNear = 200;
	private double dClipFar = 2000;
	...
	public void display( final GLAutoDrawable drawable )
	{
		...
		final Matrix4f projection = MatrixMath.screenPerspective( dCam, dClipNear, dClipFar, screenWidth, screenHeight, screenPadding, new Matrix4f() );
...
//		final int vw = viewportWidth;
		...
		{
			Vector3f nxnynz = pvm.frustumCorner( CORNER_NXNYNZ, new Vector3f() );
			Vector3f pxnynz = pvm.frustumCorner( CORNER_PXNYNZ, new Vector3f() );
			Vector3f pxpynz = pvm.frustumCorner( CORNER_PXPYNZ, new Vector3f() );
			Vector3f nxpynz = pvm.frustumCorner( CORNER_NXPYNZ, new Vector3f() );
			Vector3f nxnypz = pvm.frustumCorner( CORNER_NXNYPZ, new Vector3f() );
			Vector3f pxnypz = pvm.frustumCorner( CORNER_PXNYPZ, new Vector3f() );
			Vector3f pxpypz = pvm.frustumCorner( CORNER_PXPYPZ, new Vector3f() );
			Vector3f nxpypz = pvm.frustumCorner( CORNER_NXPYPZ, new Vector3f() );
			final Vector4f fcol = new Vector4f( 0, 1, 1, 1 );
			coloredLines.add( nxnynz, pxnynz, fcol );
			coloredLines.add( pxnynz, pxpynz, fcol );
			coloredLines.add( pxpynz, nxpynz, fcol );
			coloredLines.add( nxpynz, nxnynz, fcol );
			coloredLines.add( nxnynz, nxnypz, fcol );
			coloredLines.add( pxnynz, pxnypz, fcol );
			coloredLines.add( pxpynz, pxpypz, fcol );
			coloredLines.add( nxpynz, nxpypz, fcol );
			coloredLines.add( nxnypz, pxnypz, fcol );
			coloredLines.add( pxnypz, pxpypz, fcol );
			coloredLines.add( pxpypz, nxpypz, fcol );
			coloredLines.add( nxpypz, nxnypz, fcol );
		}
	...
	public static void main( final String[] args ) throws SpimDataException
	{
		...
		Example6 glPainter = new Example6( raiLevels, sourceTransform );
