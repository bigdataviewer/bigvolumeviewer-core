...
public class Example3 implements GLEventListener
{
	...
	public Example3( final CacheControl cacheControl, final Runnable requestRepaint )
	{
		...
		final TextureBlockCache.BlockLoader< BlockKey > blockLoader = new TextureBlockCache.BlockLoader< BlockKey >()
		{
			...
			{
				return Example3.this.loadBlock( key, buffer );
			}

			@Override
			public boolean canLoadBlock( final BlockKey key )
			{
				return Example3.this.canLoadBlock( key );
			}
		};
	...
	private final double dCam = 1000;
	private final double dClip = 200;
...
//		final double min = 0; // APR zebra.h5
//		final double max = 65535;
		final double min = 0; // APR zebra7GB.h5
...
//		System.out.println( "lookup texture took " + ( t1 - t0 ) + " ms to compute, " + ( t2 - t1 ) + " ms to upload" );
	...
	public static void main2( final SpimDataMinimal spimData )
	{
