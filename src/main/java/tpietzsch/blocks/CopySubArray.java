...
public class CopySubArray
{
	...
	public static void copysubarray1dn(
			...
			final int csx )
	{
		ByteUtils.copyShorts( src, dst + dox, sox, csx );
	}
	...
	{
		for ( int z = 0; z < csz; ++z )
			fillsubarray2db( src, dst, dox, doy + doz * dsy + z * dsy, dsx, csx, csy );
	}

	public static void fillsubarray2db(
			final short src,
			final long dst,
			final int dox,
			final int doy,
	...
	public static void fillsubarray1dn(
			...
			final int csx )
	{
		ByteUtils.setShorts( src, dst + dox, csx );
	}
}
