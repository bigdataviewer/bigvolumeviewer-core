...
import tpietzsch.util.Listeners;

/**
 * Changes an {@link AffineTransform3D}
...
public class TransformHandler
{
	 ...
	 */
	private final Listeners.List< TransformListener > listeners = new Listeners.SynchronizedList<>( l -> {
		synchronized ( affine )
		{
			l.transformChanged( affine );
		}
	} );
	...
	public Listeners< TransformListener > listeners()
	{
		return listeners;
	}

	/**
	 * notifies listeners that the current transform changed.
	 */
	private void notifyTransformChanged()
	{
		listeners.list.forEach( l -> l.transformChanged( affine ) );
	}
	...
	private class SelectRotationAxis extends AbstractNamedBehaviour implements ClickBehaviour
	{
		...
		{
			TransformHandler.this.axis = axis;
		}
	}
