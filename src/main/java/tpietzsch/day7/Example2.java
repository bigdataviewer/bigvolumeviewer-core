...
public class Example2 implements GLEventListener
{
	...
	public void init( final GLAutoDrawable drawable )
	{
		...
		gl.glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, fbWidth, fbHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, null );
		...
		*/
		...
		gl.glBindTexture( GL_TEXTURE_2D, texColorBuffer );
		byte[] rgbBytes = new byte[ fbWidth * fbHeight * 3 ];
		gl.glGetTexImage( GL_TEXTURE_2D, 0, GL_RGB, GL_UNSIGNED_BYTE, ByteBuffer.wrap( rgbBytes ) );
		gl.glBindTexture( GL_TEXTURE_2D, 0 );

		final Img< UnsignedByteType > img = ArrayImgs.unsignedBytes( rgbBytes, 3, fbWidth, fbHeight );
