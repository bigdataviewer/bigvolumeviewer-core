...
void main()
{
    ...
    // $repeat:{visC,intersectBoundingBoxC|
    bool visC = false;
    intersectBoundingBoxC( wfront, wback, n, f );
    f = min( tmax, f );
    if ( n < f )
    {
    ...
    // }$
    // $repeat:{visS,intersectBoundingBoxS|
    bool visS = false;
    intersectBoundingBoxS( wfront, wback, n, f );
    f = min( tmax, f );
    if ( n < f )
    {
        tnear = min( tnear, max( 0, n ) );
	...
	if ( tnear < tfar )
	{
		...
		for ( int i = 0; i < numSteps; ++i, step += nw + step * fwnw )
		{
            ...
            // $repeat:{visC,blockTexture,convertC|
            if ( visC )
            {
